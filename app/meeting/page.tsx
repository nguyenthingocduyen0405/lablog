...
export default function MeetingPage() {
  ...
  useEffect(() => {
    ...
    }).catch(() => setMessage("Project를 불러오지 못했어요. 최신 Supabase migration을 확인해 주세요."));
  ...
  async function submitProject(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("Project를 만들고 멤버에게 초대를 보냈어요.");
  ...
  if (!user) return <main className="flex min-h-screen items-center justify-center bg-[#f5f3ee]"><p className="text-sm font-black text-stone-400">PROJECT LOADING...</p></main>;
    ...
    <main className="min-h-screen bg-[#f5f3ee] pb-24 text-stone-950 md:pb-0">
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
        <header className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div><p className="text-xs font-black uppercase tracking-[.22em] text-emerald-600">TEAM COLLABORATION</p><h1 className="mt-3 text-4xl font-black tracking-[-.055em] sm:text-6xl">Project</h1><p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-stone-500">업무를 나누고 deadline을 지키며, 프로젝트를 끝내면 팀 전체가 함께 포인트를 받아요.</p></div>
          <button type="button" onClick={() => { setSelectedFriendIds([]); setShowCreate(true); }} className="w-fit rounded-full bg-stone-950 px-5 py-3.5 text-sm font-black text-white shadow-[0_6px_0_#2d9a73]">+ Project 만들기</button>
        </header>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[18rem_1fr]">
          ...
          <section>{selectedProject ? <div className="space-y-5">
            ...
            <div className="rounded-[1.75rem] bg-white p-5 shadow-sm ring-1 ring-black/[0.05] sm:p-6"><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-sky-600">FINAL MEETING NOTES</p><h3 className="mt-1 text-2xl font-black">최종 회의록</h3><p className="mt-2 text-xs font-semibold text-stone-400">가장 최근에 종료된 회의의 최종 저장본 한 개만 보여줘요.</p></div><span className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-black text-sky-700">FINAL</span></div><div className="mt-5">{meetingHistory.length === 0 ? <div className="rounded-2xl border border-dashed border-stone-200 px-5 py-9 text-center"><p className="text-3xl">📝</p><p className="mt-3 text-sm font-black text-stone-500">아직 저장된 최종 회의록이 없어요.</p></div> : meetingHistory.map((meeting) => <details key={meeting.id} open className="group rounded-2xl bg-[#faf9f6] ring-1 ring-black/[0.05]"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-4"><div><h4 className="text-sm font-black text-stone-800">{meeting.title}</h4><p className="mt-1 text-[10px] font-bold text-stone-400">{formatDateTime(meeting.startsAt)} · 종료 {formatDateTime(meeting.endedAt)}</p></div><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-lg font-bold text-stone-400 ring-1 ring-black/[0.06] transition group-open:rotate-45">+</span></summary><div className="border-t border-black/[0.05] px-4 py-5"><p className="whitespace-pre-wrap text-sm font-semibold leading-7 text-stone-600">{meeting.notes || "작성된 회의 내용이 없어요."}</p>{meeting.notesUpdatedAt && <p className="mt-4 text-[10px] font-bold text-stone-300">마지막 저장 {formatDateTime(meeting.notesUpdatedAt)}</p>}</div></details>)}</div></div>
