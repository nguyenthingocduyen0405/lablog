...
export default function MeetingPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    loadTeamProjectTasks(selectedProjectId).then((items) => { if (!cancelled) setTasks(items); })
      .catch(() => { if (!cancelled) setMessage("업무 목록을 불러오지 못했어요."); });
    return () => { cancelled = true; };
  }, [selectedProjectId]);
  ...
  async function startMeeting() {
    if (!selectedProject || !isHost || isSaving) return;
    setIsSaving(true);
    try { const meeting = await startProjectMeeting(selectedProject.id, `${selectedProject.name} 팀 미팅`); setLiveMeeting(meeting); setActiveMeeting(meeting); }
  ...
  async function endMeeting() {
    ...
    setIsSaving(true);
    try { await endProjectMeeting(activeMeeting.id, activeMeeting.projectId); setActiveMeeting(null); setLiveMeeting(null); }
