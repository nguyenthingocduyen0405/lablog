    ...
    <main className="min-h-screen bg-[#f5f3ee] pb-24 text-stone-950 md:pb-0">
      <header className="relative z-[60] border-b border-black/[0.06] bg-[#f5f3ee]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-5 py-4 sm:px-8">
          <Link href={currentUser.chapterTwoCompletedAt ? "/update#feed" : "/labquest?chapter=2&locked=feed"} className="flex items-center gap-2 text-sm font-black transition hover:-translate-x-1">{currentUser.chapterTwoCompletedAt ? "← 랩 피드" : "🔒 랩 피드"}</Link>
      ...
      <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-12">
        <section className={`relative overflow-visible rounded-[2.5rem] bg-[#181611] px-6 py-8 text-white shadow-[0_24px_80px_rgba(40,32,14,0.18)] sm:px-10 sm:py-10 ${isMe && !currentUser.chapterTwoCompletedAt ? "mb-36" : ""}`}>
          ...
          <div className="relative flex flex-col gap-7 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-3">
              ...
              <div className="relative min-w-24 rounded-2xl bg-violet-500/20 px-5 py-4 text-center"><p className="text-2xl font-black">{totalScore}P</p><p className="text-[10px] font-bold uppercase tracking-widest text-white/45">score</p>{isMe && !currentUser.chapterTwoCompletedAt && <div className="absolute right-0 top-[calc(100%+1rem)] z-30 w-72 rounded-2xl bg-white p-5 text-left text-stone-950 shadow-[0_18px_50px_rgba(0,0,0,.3)] before:absolute before:-top-2 before:right-9 before:h-4 before:w-4 before:rotate-45 before:bg-white"><p className="text-[10px] font-black tracking-[.16em] text-violet-500">SCORE GUIDE</p><p className="mt-2 text-sm font-bold leading-6">포인트를 모으고 싶나요? Chapter 2 미션을 완료하면 Mission, Update, Feed, Team 기능이 열려요.</p><Link href="/labquest?chapter=2&locked=score" className="mt-4 inline-flex w-full items-center justify-center rounded-full bg-stone-950 px-5 py-3 text-sm font-black text-white shadow-[0_5px_0_#c7a600]">시작하기</Link></div>}</div>
              ...
              {isMe && <Link href={currentUser.chapterTwoCompletedAt ? "/update#new-post" : "/labquest?chapter=2&locked=update"} className="flex items-center rounded-2xl bg-[#ffd84d] px-5 py-3 text-sm font-black text-stone-950 transition hover:-translate-y-0.5">{currentUser.chapterTwoCompletedAt ? "새 기록 올리기" : "🔒 새 기록 올리기"}</Link>}
            </div>
          </div>
        </section>
        ...
        <section className="mt-10">
          ...
          {memberPosts.length > 0 ? (
            ...
            <div className="rounded-[2rem] border-2 border-dashed border-stone-300 bg-white/45 px-6 py-16 text-center">
              ...
              {isMe && <Link href={currentUser.chapterTwoCompletedAt ? "/update#new-post" : "/labquest?chapter=2&locked=update"} className="mt-6 inline-flex rounded-full bg-stone-950 px-5 py-3 text-sm font-black text-white">{currentUser.chapterTwoCompletedAt ? "기록 올리기" : "🔒 기록 올리기"}</Link>}
            </div>
          )}
        </section>

        {currentUser.chapterTwoCompletedAt ? <section className="mt-14 border-t border-black/[0.07] pt-8">
        ...
        </section> : isMe && <section className="mt-14 rounded-[1.75rem] border border-stone-200 bg-stone-100 p-6 text-stone-400"><p className="text-xs font-black tracking-[.16em]">🔒 TEAM · CHAPTER 2</p><p className="mt-2 text-sm font-semibold">Chapter 2를 완료하면 다른 랩 멤버의 프로필을 볼 수 있어요.</p></section>}
      </div>
    </main>
  );
}
