        ...
        <div className="absolute right-0 top-14 z-50 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-[1.5rem] bg-white shadow-[0_22px_70px_rgba(35,31,24,.22)] ring-1 ring-black/[0.08]">
          ...
          <div className="max-h-96 overflow-y-auto">
            {notifications.length === 0 ? (
              ...
              <Link key={item.id} href={item.type === "team_project_invite" ? "/meeting" : item.type.endsWith("_reminder") ? "/update#new-post" : `/update#post-${item.postId}`} onClick={() => setIsOpen(false)} className="flex gap-3 border-b border-stone-100 px-4 py-4 transition last:border-0 hover:bg-[#fff9df]">
