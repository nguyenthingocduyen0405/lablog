...
and asks the loopback-only Ollama service for a structured quiz in the
requester's current interface language. It does not call OpenAI or another
paid model API.

The default `qwen3:4b` model is about 2.5 GB. Generation is CPU-only on this
JCloud host. The worker generates all ten questions in one bounded
single-language request, which can take roughly 10–15 minutes.
...
The model unloads after each job. The worker heartbeats the queue before every
model request so a valid long job cannot be reclaimed while it is generating.
